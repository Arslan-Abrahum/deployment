...
export default function LiveAuctionsTab() {
  ...
  const filteredData = useMemo(() => {
    return SAMPLE_DATA.filter((item) => {
      ...
      const matchDate = true; // Add date filtering if needed
        ...
        <table className="auction-table">
          ...
          <tbody>
            {paginatedData.map((item) => (
              <tr key={item.id} style={{ cursor: "pointer" }} onClick={() => navigate("/AdminAuctionResults")}>
                <td>{item.id}</td>
                <td>{item.user}</td>
      ...
      {filteredData.length > 0 && (
        <div className="table-pagination">
          ...
          <div className="pagination-controls">
            <button className="pagination-btn prev" onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={page === 1}>.</button>
            ...
            <button className="pagination-btn next" style={{color:"black"}} onClick={() => setPage(p => Math.min(p + 1, totalPages))} disabled={page === totalPages}>.</button>
          </div>
        </div>
      )}
    </div>
